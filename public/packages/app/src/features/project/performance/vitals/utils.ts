...
export const calculateHealthPercentage = (
  ...
  const result = Object.entries(threshold).reduce((acc, [key, range]) => {
    ...
    const count =
      list.filter((t) => t.value >= thresholdRange.min && t.value < thresholdRange.max)?.length ||
      0;
...
export const parseToBins = (perfs: Performance[]) => {
  ...
  const pushToBins = (values: number[], INTERVAL = 100, MAX_VALUE = 100) => {
    ...
    for (let i = 0; i <= MAX; i += INTERVAL) {
      const count = values.filter((value) => value >= i && value <= i + INTERVAL).length;
...
export const calculatePercentile = <T extends PercentileCoreType>(
  data: T[],
  percentile = 0.75
) => {
  if (!data) {
